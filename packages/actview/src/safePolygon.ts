...
/**
 * actview 版（upstream 为 React 工具函数，非 hook——不在组件中调用）。
 *
 * 与 upstream 的差异：
 * - `const timeoutRef = {current: -1}`（React 遗留风格）改为局部 `timeoutId`
 *   变量 + 内联 `clearTimeout`（`clearTimeoutIfSet` 需要框架 `Ref<number>`，
 *   此处无框架 ref 语义，直接管理更清晰）
 * - `HandleCloseContext` 的 `elements` 字段为 `Ref` → `.value`；
 *   `x` / `y` / `nodeId` 为 `Ref` → `.value`；`placement` 为 `Ref` → `.value`
 */
...
export function safePolygon(options: SafePolygonOptions = {}) {
  ...
  const fn: HandleClose = ({
    ...
    return function onMouseMove(event: MouseEvent) {
      ...
      const isOverFloatingEl = contains(elements.floating.value, target);
      const isOverReferenceEl = contains(elements.domReference.value, target);
      const refRect = elements.domReference.value.getBoundingClientRect();
      const rect = elements.floating.value.getBoundingClientRect();
      ...
      if (
        ...
        contains(elements.floating.value, event.relatedTarget)
      ) {
        return;
      }
      ...
      if (
        tree &&
        getNodeChildren(tree.nodesRef.value, nodeId.value).length
      ) {
        return;
      }
