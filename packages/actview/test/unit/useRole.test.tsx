...
const AppWithExternalRef = defineComponent(function (
  props: UseRoleProps & {initiallyOpen?: boolean},
) {
  const open = ref(props.initiallyOpen ?? false);
  const nodeId = useId();
  const {refs, context} = useFloating({
...
describe('dialog', () => {
  ...
  test('sets correct aria attributes with external ref, multiple useRole calls', async () => {
