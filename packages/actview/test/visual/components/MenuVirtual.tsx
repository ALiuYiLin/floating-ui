...
export const MenuComponent = defineComponent(function (
  props: MenuProps & any,
) {
    ...
    const child = (
      isNested ? (
        <div
          ...
          className={`text-left flex gap-4 justify-between items-center rounded py-1 px-2 cursor-default${
            parentActive === item.index.value ? ' bg-red-500 text-white' : ''
          }${isNested ? ' focus:bg-red-500 outline-none' : ''}${
          ...
          {...getReferenceProps({
            onKeyDown(event: any) {
              if (event.key === ' ' || event.key === 'Enter') {
                console.log('clicked', props.virtualItemRef.value);
              }
            },
          })}
        />
  ...
  return () => {
    ...
    return (
      <div
        ...
        className={`text-left flex py-1 px-2 focus:bg-red-500 outline-none rounded cursor-default${
